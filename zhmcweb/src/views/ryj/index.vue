  ...
  <div id="screen" class="screen-1920">
    ...
    <header class="screen-header">
      <div class="brand">
        ...
        <span class="brand-text">热应激指数</span>
      </div>
      <a-date-picker
          class="dark-picker"
    ...
    <main class="screen-main">
      ...
      <section class="card chart-card">
        <div class="card-title">室外环境（热应激趋势）</div>
      ...
      <!-- 下：复制的室内图表 -->
      <section class="card chart-card">
        <div class="card-title">室内环境（热应激趋势）</div>
...
function setOptionForChart(c, s) {
  ...
  c.setOption({
    ...
    color: ['#FF8F00', '#FF5252', '#00BFA5'],
    tooltip: {
      trigger: 'axis',
      axisPointer: { type: 'line', lineStyle: { color: 'rgba(255,138,101,.85)', width: 1 } },
      backgroundColor: 'rgba(32, 14, 10, .92)',
      borderColor: 'rgba(255, 138, 101, .35)',
      borderWidth: 1,
    ...
    series: [
      {
        ...
        lineStyle: { width: 2.5, color: '#1E90FF', shadowBlur: 12, shadowColor: 'rgba(0,200,83,.35)' },
        data: s.yTHI,
      ...
      },
      {
        ...
        symbol: 'circle',
        symbolSize: 6,
        lineStyle: { width: 2.5, color: '#FF5252', shadowBlur: 10, shadowColor: '#FF5252' },
        areaStyle: {
          color: new echarts.graphic.LinearGradient(0, 0, 0, 1, [
            { offset: 0, color: 'rgba(255,82,82,.25)' },
            { offset: 1, color: 'rgba(255,82,82,.04)' }
          ])
        },
        data: s.yTemp,
        emphasis: { focus: 'series' }
      },
      {
        ...
        color: '#228B22',
        symbol: 'none',
        lineStyle: { type: 'dashed', width: 1.8, color: '#228B22' },
        data: s.yRH,
        emphasis: { focus: 'series' }
      }
    ]
...
:host, .screen-1920{
  background:
    radial-gradient(1200px 700px at 15% -10%, #5a2315, transparent 60%),
    radial-gradient(900px 600px at 85% 110%, #5a2315, transparent 60%),
    linear-gradient(180deg, #2b1510 0%, #2b1510 100%);
  color: var(--txt);
...
.screen-header{
  ...
  background:
    linear-gradient(90deg, rgba(255,138,101,.18), transparent 40%),
    linear-gradient(180deg, rgba(255,255,255,.02), rgba(255,255,255,0));
}
.brand{ display:flex; align-items:center; gap:10px; font-size:28px; font-weight:700; letter-spacing:.5px; }
.logo-dot{
  width:12px; height:12px; border-radius:50%;
  background: radial-gradient(circle, var(--acc), rgba(255,82,82,.2));
  box-shadow: 0 0 16px rgba(255,82,82,.7), 0 0 24px rgba(255,82,82,.25);
}
.brand-text{ color:#FFEFE6; text-shadow: 0 0 12px rgba(255,138,101,.38); }
...
.station-btn:hover{ transform: translateY(-1px); box-shadow: 0 6px 18px rgba(0,0,0,.35); border-color: rgba(255,138,101,.55); }
.station-btn.active{
  color: #0A1220; border-color: transparent;
  background: linear-gradient(180deg, #FF8F00, #FF7043);
  box-shadow: 0 0 22px rgba(255,138,101,.35), inset 0 1px 0 rgba(255,255,255,.18);
}
...
.btn-ghost{
  ...
  border:1px solid rgba(255,138,101,.55); color:#FFEFE6; background:rgba(255,138,101,.12);
  box-shadow:0 0 12px rgba(255,138,101,.30) inset, 0 0 16px rgba(255,138,101,.20);
  transition:transform .2s ease, box-shadow .2s ease, opacity .2s ease;
  font-size: 13px;
}
.btn-ghost:hover{ transform:translateY(-1px); box-shadow:0 0 16px rgba(255,138,101,.36) inset, 0 6px 18px rgba(0,0,0,.4); }
